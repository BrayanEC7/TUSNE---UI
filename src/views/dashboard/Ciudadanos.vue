    ...
    <div class="surface-section pb-5" v-if="!visible">
        <div class="px-4 py-5 md:px-6 lg:px-7">
            <ul class="list-none p-0 m-0 flex align-items-center font-medium mb-3">
                <li><span class="text-700 font-light">Registro de Ciudadano</span></li>
            </ul>
            <div class="font-medium text-3xl text-900 mb-3">Ciudadano</div>
            <div class="grid">
            ...
            <div v-else>
                ...
                <div v-else>
                    <DataTable :value="listamenus" tableStyle="min-width: 50rem">
                        ...
                        <Column field="denominacionPers" header="Denominación" sortable style="width: 25%"></Column>
                        <Column field="asociacionPers" header="Razon Social" sortable style="width: 25%"></Column>
                        <Column field="cargoPers" header="Cargo" sortable style="width: 25%"></Column>
    ...
    <Dialog v-model:visible="visible" modal :style="{ width: '50rem' }"
        ...
        <template #header>
            <p class="bg-gray-100 text-800 font-bold block border-bottom-1 surface-border p-3 text-2xl w-full">
                <i class="pi pi-user text-green-400 text-2xl mr-2"></i> Nuevo Ciudadano
            </p>
        </template>
        <div class="grid py-4">
            ...
            <div class="col-12 lg:col-6">
                <label for="Denominación" class="font-bold block mb-2"> Denominación </label>
                <Dropdown v-model="denominacionPers" :options="denominacionOpcion" optionLabel="nombre" optionValue="id"
                    placeholder="Seleccionar una opción" checkmark class="w-full" />
            </div>
            <div class="col-12 lg:col-6">
                <label for="Asociacion" class="font-bold block mb-2"> Razón Social </label>
                <InputText type="text" v-model="asociacionPers" class="w-full" :disabled="isPersonaNatural" />
            </div>
            <div class="col-12 lg:col-6">
                <label for="Cargo" class="font-bold block mb-2"> Cargo </label>
                <InputText type="text" v-model="cargoPers" class="w-full" :disabled="isPersonaNatural" />
            </div>
            <div class="col-12 lg:col-6">
...
export default {
    ...
    data() {
        return {
            ...
            filtroBusqueda: '',
            id: '',
            rolId: '41cf0a15-1f10-4953-ad15-4067e3a95477',
            nombrePers: '',
            dniPers: '',
            cargoPers: '',
            denominacionPers: '',
            ...
            denominacionOpcion: [
                ...
                { id: "ASOCIACIÓN", nombre: "ASOCIACIÓN" }
            ]
        };
    },
    computed: {
        ...
        listamenus() {
            return this.listamenus.filter(item => item.nombre_rol === 'Ciudadano');
        },
        isPersonaNatural() {
            // Devuelve verdadero si la denominación es "Persona Natural"
            return this.denominacionPers === "PERSONA NATURAL";
        }
    },
    methods: {
        ...
        validateFields() {
            ...
            if (["PERSONA JURÍDICA", "ASOCIACIÓN"].includes(this.denominacionPers) && !this.asociacionPers) {
                this.$toast.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: 'La Razón Social es obligatoria para Persona Jurídica y Asociación.',
                    life: 5000,
                });
                return false;
            }

            // Validación para el campo "Cargo"
            if (["PERSONA JURÍDICA", "ASOCIACIÓN"].includes(this.denominacionPers) && !this.cargoPers) {
                this.$toast.add({
                    severity: 'error',
                    summary: 'Error',
                    detail: 'El Cargo es obligatorio para Persona Jurídica y Asociación.',
                    life: 5000,
                });
                return false;
            }
        ...
        filtrarMenus() {
            ...
            if (searchTerm) {
                const datosM = this.listamenus.filter((fila) => fila.nombrePers.toLowerCase().includes(searchTerm) || fila.apellido_patPers.toLowerCase().includes(searchTerm) || fila.apellido_matPers.toLowerCase().includes(searchTerm) || fila.dniPers.includes(searchTerm));
        ...
        async eliminarMenu(idtemp) {
            try {
                ...
                this.confirm.require({
                    group: 'positioned',
                    message: 'Eliminar al vendedor podría ocasionar la pérdida de información sobre las propiedades.',
